...
class TestApiMethods:
    ...
    def test_get_daily_returns_dataframe(self, client):
        ...
        client._pro.daily.return_value = mock_df
        ...
        client._pro.daily.assert_called_once_with(
            ts_code="600519.SH", start_date="20260101", end_date="20260115"
        )

    def test_get_daily_exception_returns_none(self, client):
        """get_daily 异常时返回 None"""
        client._pro = MagicMock()
        client._pro.daily.side_effect = Exception("API error")
        result = client.get_daily("600519.SH", "20260101", "20260115")
        assert result is None

    def test_get_daily_by_trade_date_returns_full_market(self, client):
        ...
        client._pro = MagicMock()
        client._pro.daily.return_value = mock_df
        result = client.get_daily_by_trade_date("20260115")
        assert len(result) == 2
        client._pro.daily.assert_called_once_with(trade_date="20260115")
    ...
    def test_check_connection_failure(self, client):
        client._pro = MagicMock()
        client._pro.daily.return_value = pd.DataFrame()
        assert client.check_connection() is False

    def test_check_connection_none(self, client):
        client._pro = MagicMock()
        client._pro.daily.return_value = None
        assert client.check_connection() is False
