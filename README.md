...
看板的「资料库」页原样展示 HTML 文件：方案文档（如 `docs/dsh-integration.html`）和你自己写的
公司 / 行业研报。文件放进目录、点刷新就能看到，不需要重新构建前端。默认扫两个目录：

| 目录 | 标签 | 说明 |
|---|---|---|
| `docs/` | 方案文档 | 入库；`docs/archive/` 不扫 |
| `reports/` | 研报 | 被 `.gitignore` 忽略，适合放不想公开的个人研报 |

子目录名会成为分类（`reports/公司/xxx.html` 显示为「研报 · 公司」）；研报引用的图片、CSS
放在旁边用相对路径即可。目录和标签可用 `.env` 里的 `LIBRARY_DIRS` 改，格式 `路径=标签;路径=标签`。
