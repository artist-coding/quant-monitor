...
def add_watchlist(req: WatchlistAddRequest):
    """添加到自选股"""
    try:
        return watchlist_service.add_to_watchlist(req.ts_code, req.tags, req.notes)
...
def refresh_watchlist(days: int = Query(default=250, ge=120, le=500)):
    """补齐自选股历史日线并重算量化指标"""
    try:
        return watchlist_service.refresh_watchlist(days=days)
    except Exception as e:
