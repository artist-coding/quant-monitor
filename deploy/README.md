...
| `quant-monitor-amv.timer.user` | **user** | zx | — | 周一至周五 20:05 + 22:05 触发上面那条 |
...
但**交易日历要提前备好**——`trade_cal` 接口限流 1 次/分钟且限流时静默返回空,
...
活跃市值排 20:05,日线排 20:00,刻意错开:

1. 脚本要拿 `daily_kline` 的最新交易日判断活跃市值有没有落后。日线还没入库时
   两边一样旧,落后天数恒为 0,那道防线等于没有——不报错,只是不再报警。
2. 两条同步写的是同一个 SQLite 主库,同时起容易撞 `database is locked`。

`quant-monitor-amv.service` 里还写了 `After=quant-monitor-sync.service` 做第二道
保险:两个 job 万一排进同一个事务(比如关机错过后开机一起补跑),systemd 会按序跑。
刻意不用 `Requires=`——那会让每次活跃市值同步都顺带拉一次日线同步。

22:05 那次兜底,和日线同步的两触发点是同一个思路。

**一天最多两次转存,别再加触发点**:BaiduPCS-Go 是非官方客户端,
频率高了会触发百度的人机验证。
