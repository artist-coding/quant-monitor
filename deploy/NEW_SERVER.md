...
```bash
zt sync init                                          # 建表
zt sync trade-cal --start 20170101 --end 20171231     # 逐年，每次之间等满 1 分钟
zt sync trade-cal --start 20180101 --end 20181231
# ... 一直到明年
```

交易日历这一步**必须自己核对行数**：接口限流 1 次/分钟，而且**限流时静默返回空**——
不报错，只是写进去 0 行。一年 365/366 行（`trade_cal` 存的是自然日，`is_open` 有 0 有 1），
差太多就是被限流了，等一分钟重来。**按年查，别只看 `MAX(cal_date)`**：
...
整年整年地缺是这个接口最典型的坏法——限流时它不报错、只返回空，那一年就一行都没写进去，
而首尾两年还在，`MIN`/`MAX` 看着完全正常。**现在的库里就有这个洞：2018、2024、2025 三年是空的**
（2017、2019~2023、2026 齐全）。日常日线同步不受影响（它只需要"库里最新日线 → 今天"这一段有日历），
但跨这几年的历史回补和任何拿 `trade_cal` 数交易日的地方都会算错。走路线 A 恢复备份的话，
这个洞也一起搬到新机上，要补就逐年重跑 `zt sync trade-cal`。
