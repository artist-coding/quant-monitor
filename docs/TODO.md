...
### 终选 Agent（deepseek-harness 接入）

方案、契约、阶段与完成标准见 `docs/dsh-integration.html`（2026-09-04 起为这条线的唯一口径）。
此前的模拟器 v0.5～v1.0 路线（含「与 SKILL.md LLM 层打通」）随 e47a1b8 删除 simulator 子系统一并作废，不再列出。

- [ ] 阶段 0：`agent_verdicts` 契约 + `final_picks` 接线 + 候选池
- [ ] 阶段 1：K 线特征摘要 `modules/chart_facts.py`（纯文本，不用多模态）
- [ ] 阶段 2：离线回放评估（匿名候选、无网，按 verdict 分组算 30 日收益）
- [ ] 阶段 3：dsh headless 接入（只读 MCP 工具 / skill / 每票固定清单）
- [ ] 阶段 4：影子模式 + 分歧回流 + 规则下沉；PIT 快照任务不排队、尽早上线
...
| **v3.7.0+** | 终选 Agent（deepseek-harness 接入），见 `docs/dsh-integration.html` | 📋 待实施 |
