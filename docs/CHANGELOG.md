...
- 新增 `zt sync market-daily`：按交易日一次获取全 A 股未复权日线并幂等写入 SQLite。
- 同步前使用 Tushare 交易日历判断开市状态，节假日自动跳过，空响应按失败记录。
- 每日同步可同时 upsert 最新上市股票基础信息，避免重复刷新触发主键冲突。
- 新增 Linux systemd service/timer，默认工作日北京时间 16:30 运行。
- 本地 `sync status` 不再依赖 Tushare 配置，空数据库会自动初始化后返回状态。

### 自选股行情与系统指标面板

- 自选股支持直接输入 6 位代码，自动补全交易所后缀并从股票基础表识别名称；已有裸代码记录会自动迁移。
- 自选股接口新增最新价、涨跌幅、数据日期、历史日线数量，以及综合/B1/趋势/量价/风险评分。
- 看板新增 J 值、量比、MACD、趋势状态与交易信号展示，移除低价值的添加日期列。
- 新增“刷新行情与指标”，按需补齐每只自选股的历史日线并重算指标缓存。
- 新增股票核心信息召回：添加时直接查询本地 `stock_basic`、最新 `daily_kline` 和 `indicator_cache`，将完整条目随接口响应返回并立即写入前端缓存。
