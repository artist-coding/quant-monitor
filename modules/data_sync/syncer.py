...
class DataSyncer:
    ...
    def _call_api_with_retry(self, api_name: str, func, *args, **kwargs):
        ...
        **限流错误不重试**：Tushare 的配额是按接口分别计的，且颗粒度很粗
        （实测 index_daily 5 次/天、stock_basic 1 次/小时、trade_cal 1 次/小时且 5 次/天）。
    ...
    def sync_trade_cal(self, start_date: str, end_date: str, exchange: str = "SSE") -> int:
        ...
        Tushare 的 trade_cal 是**两层配额叠加：1 次/小时 + 5 次/天**（2026-09-01 实测，
        上游两条原文都见过），而且**被拒的请求同样计入日额度**——当天试崩了就没机会补救，
        只能等次日。因此调用方必须按"整年"为粒度拉取，每次之间隔 62 分钟，
        一天最多补 4~5 年。入库使用 INSERT OR REPLACE，重复执行幂等。

        被限流时本方法返回 0 并写一条 failed 的 sync_log。**注意返回值和"这段区间确实没数据"
        无法区分**——真正难查的不是当场（日志里有上游原文），而是事后：某一年整年没写进去，
        ``MIN``/``MAX(cal_date)`` 看着完全正常，只有按年 GROUP BY 数行数才暴露。
