...
class DataSyncer:
    ...
    def _call_api_with_retry(self, api_name: str, func, *args, **kwargs):
        ...
        **限流错误不重试**：Tushare 的配额是按接口分别计的，且颗粒度很粗
        （实测 index_daily 5 次/天、stock_basic 1 次/小时、trade_cal 1 次/小时）。
    ...
    def sync_trade_cal(self, start_date: str, end_date: str, exchange: str = "SSE") -> int:
        ...
        Tushare 的 trade_cal 接口被限流到 **1 次/小时**（2026-09-01 实测，上游原文
        「您访问接口(trade_cal)频率超限(1次/小时)」），因此调用方必须按"整年"为粒度
        拉取，且多年回补时每年之间要隔一小时。入库使用 INSERT OR REPLACE，重复
        执行幂等。
