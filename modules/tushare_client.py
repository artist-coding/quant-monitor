...
class TushareClient:
    ...
    def _is_permanent_error(exc: Exception) -> bool:
        ...
        限流同理：退避 1s/2s 远小于任何限流窗口（trade_cal 实测 1 次/小时且 5 次/天），
        窗口内重试必然再失败，反而更快烧光配额——被拒的请求照样计入日额度。
        """
        msg = str(exc)
    ...
    def get_daily_by_trade_date(self, trade_date: str) -> pd.DataFrame | None:
        ...
        Tushare ``daily`` 接口支持仅传 ``trade_date`` 返回当日全市场数据，
        适合收盘后的每日增量归档，避免逐只股票请求。
        """
        return self._call_api_with_retry(
