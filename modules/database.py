...
def init_database(verbose: bool = True) -> None:
    ...
    with get_connection() as conn:
        ...
        # 12. 交易日历本地缓存表
        # Tushare trade_cal 接口限流严重（实测 1 次/小时 + 5 次/天），本地缓存后避免每日同步反复请求
        cursor.execute("""
...
def add_watchlist_item(ts_code: str, name: str = "", tags: str = "", notes: str = "") -> int:
    ...
    with get_connection() as conn:
        ...
        row = cursor.execute("SELECT id FROM watchlist WHERE ts_code = ?", (ts_code,)).fetchone()
...
def get_stock_core_info(ts_code: str) -> dict[str, Any] | None:
    """从本地数据库召回股票名称、最新行情和已有指标快照。"""
    with get_connection() as conn:
        row = conn.execute(
            """
            SELECT
                ...
                k.vol,
                k.amount,
                (SELECT COUNT(*) FROM daily_kline kh WHERE kh.ts_code = b.ts_code) AS kline_count,
                i.trade_date AS indicator_trade_date,
                i.j AS cached_j,
                i.vol_ratio AS cached_vol_ratio,
                i.dif AS cached_dif,
                i.dea AS cached_dea,
                i.signal AS cached_signal
            FROM stock_basic b
            LEFT JOIN daily_kline k
              ON k.ts_code = b.ts_code
             AND k.trade_date = (
                 SELECT MAX(k2.trade_date) FROM daily_kline k2 WHERE k2.ts_code = b.ts_code
             )
            LEFT JOIN indicator_cache i
              ON i.ts_code = b.ts_code
             AND i.trade_date = (
                 SELECT MAX(i2.trade_date) FROM indicator_cache i2 WHERE i2.ts_code = b.ts_code
             )
            WHERE b.ts_code = ?
            """,
            (ts_code,),
    ...
    with get_connection() as conn:
        ...
        sql = """
            SELECT
                w.id,
                w.ts_code,
                COALESCE(NULLIF(w.name, ''), b.name, '') AS name,
                w.tags,
                w.notes,
                w.added_date,
                w.alert_enabled,
                ...
                k.vol,
                k.amount,
                (SELECT COUNT(*) FROM daily_kline kh WHERE kh.ts_code = w.ts_code) AS kline_count
            FROM watchlist w
            LEFT JOIN stock_basic b ON b.ts_code = w.ts_code
            LEFT JOIN daily_kline k
              ON k.ts_code = w.ts_code
             AND k.trade_date = (
                 SELECT MAX(k2.trade_date) FROM daily_kline k2 WHERE k2.ts_code = w.ts_code
             )
        """
        params: tuple[str, ...] = ()
        if tags:
            sql += " WHERE w.tags LIKE ?"
            params = (f"%{tags}%",)
