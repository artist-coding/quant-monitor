...
export default function KlineChart({ data, height = 820 }: Props) {
  ...
  const option = {
    ...
    dataZoom: [
      ...
      { type: 'slider', xAxisIndex: [0, 1, 2, 3, 4, 5], bottom: 5, height: 15, borderColor: 'rgba(148, 163, 184, 0.2)', backgroundColor: 'rgba(9, 12, 22, 0.5)', fillerColor: 'rgba(245, 185, 66, 0.12)', handleStyle: { color: CHART_COLORS.gold }, moveHandleStyle: { color: CHART_COLORS.gold }, textStyle: { color: CHART_COLORS.axisLabel } },
    ],
    series: [
      {
        name: 'K线',
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between pb-3 mb-1 border-b border-border/30">
        <div className="text-[11px] text-text-muted font-medium tracking-wider uppercase">主力大势背景渲染</div>
